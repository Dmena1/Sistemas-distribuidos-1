    ...
    with sync_playwright() as p:
        ...
        def handle_response(response):
            if "georss" in response.url and response.status == 200:
                try:
                    ...
                    print(f"Eventos encontrados: {nuevos} | Eventos encontras en total: {len(eventos_acumulados)}")
        ...
        for evento in eventos_acumulados:
            try:
                ...
                print(f"Error: {e}")
        print(f"Se enviaron {enviados} eventos a http://cache:8001/eventos correctamente")
